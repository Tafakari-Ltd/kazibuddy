...
const EmployerApplicationsSection = () => {
  ...
  const { userProfile: employerProfile } = useSelector(
  ...
  const fetchEmployerApplications = async () => {
    if (!employerProfile?.id) {
      console.log(
        "EmployerApplicationsSection Debug: Cannot fetch - no employer profile ID",
      );
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      console.log(
        "EmployerApplicationsSection Debug: Fetching detailed data for",
        response.applications.length,
        "applications",
      );
      console.log(
        "EmployerApplicationsSection Debug: Current employer profile ID:",
        employerProfile.id,
      );
      ...
      const employerApplications = detailedApplications.filter((app: any) => {
        ...
        const jobEmployerId =
          app.job?.employer?.id || app.job_details?.employer;
        const isMatch = jobEmployerId === employerProfile.id;

        if (!isMatch) {
          console.log(
            `EmployerApplicationsSection Debug: Filtering out application ${app.id} - job employer ${jobEmployerId} doesn't match current employer ${employerProfile.id}`,
          );
        }
      ...
      console.log(
        "EmployerApplicationsSection Debug: Filtered applications count (only this employer's jobs):",
        employerApplications.length,
      );
      console.log(
        "EmployerApplicationsSection Debug: Total applications before filtering:",
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-blue-800 text-sm mt-1">
              Applications are reviewed by our admin team. You can view details
              here, but approval decisions are made by administrators to ensure
              fair and consistent hiring processes.
            </p>
          </div>
        </div>
      </div>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div
                        className={`flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(application.status)}`}
                      >
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-600">
                      ...
                      {((typeof application.job !== "string"
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          {(typeof application.job !== "string"
                            ? application.job?.location
                            : undefined) || application.job_details?.location}
                        </span>
                      )}
            ...
            >
              <div className="flex justify-between items-start p-6 border-b">
                <div>
                  ...
                  <div
                    className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(selectedApplication.status)}`}
                  >
                  ...
                  <div>
                    ...
                    <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg">
                      <h5 className="font-medium text-blue-900 mb-2">
                        {(typeof selectedApplication.job !== "string"
                      ...
                      <div className="flex items-center gap-4 text-sm text-blue-700">
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          Budget:{" "}
                          {(typeof selectedApplication.job !== "string"
                            ? selectedApplication.job?.budget_min
                            : undefined) ||
                            selectedApplication.job_details?.budget_min}{" "}
                          -{" "}
                          {(typeof selectedApplication.job !== "string"
                            ? selectedApplication.job?.budget_max
                            : undefined) ||
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                    ...
                    <p className="text-yellow-800 text-xs">
                      Application status is managed by our admin team. You'll be

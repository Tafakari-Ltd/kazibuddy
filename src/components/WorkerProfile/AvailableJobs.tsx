...
import { useJobApplicationModal, useMyApplications } from '../../Redux/Functions/jobs';
...
interface JobWithApplicationStatus extends JobDetails {
  ...
  applicationStatus?: 'pending' | 'reviewed' | 'accepted' | 'rejected' | 'withdrawn' | null;
  hasApplied?: boolean;
}
...
export const AvailableJobs: React.FC<AvailableJobsProps> = ({
  ...
  const handleWithdrawApplication = async (jobId: string, applicationId: string) => {
    if (!confirm('Are you sure you want to withdraw your application for this job?')) {
      return;
    }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            {jobsWithStatus.length} job{jobsWithStatus.length !== 1 ? 's' : ''} available
          </p>
        </div>
        {onRefresh && (
          <button
              ...
              <div className="p-6">
                ...
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex flex-col items-end space-y-2">
                    {job.applicationStatus && getStatusBadge(job.applicationStatus)}

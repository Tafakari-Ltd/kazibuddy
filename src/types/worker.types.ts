...
export const formatAvailabilitySchedule = (
  ...
  const days = Object.keys(schedule).filter(
    (day) => schedule[day] && schedule[day].length >= 2,
  );
...
export const calculateProfileCompletion = (
  profile: Partial<CreateWorkerProfileData>,
): number => {
  ...
  const hasAvailability =
    ...
    Object.keys(profile.availability_schedule).some(
      (day) =>
        profile.availability_schedule![day] &&
        profile.availability_schedule![day].length >= 2,
    );
